...
class OrderService {
  ...
  static async processCheckout(checkoutData: CheckoutData) {
    try {
      const { userId, items, shipping_address, payment_method } = checkoutData;
      ...
      const order = await prisma.order.create({
        data: {
          ...
          shipping_address: JSON.stringify(shippingAddressWithDeliveryFee),
  ...
  static async verifyPayment(reference: string, userId: number) {
    try {
      ...
      const verificationResult = await this.verifyPaystackPayment(reference);
  ...
  static async cancelOrder(orderId: string, userId: number) {
    try {
      ...
      if (order.payment_status === "paid") {
        return {
          error: true,
          message: "Cannot cancel paid orders. Please contact support",
        };
      }
